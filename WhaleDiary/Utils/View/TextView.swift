...
class TextView: UITextView, UIDropInteractionDelegate {
    ...
    weak var t_viewController: UIViewController?
    ...
    func insertImage(_ image: UIImage) {
        ...
        if Configure.shared.imageStorage == .local {
            ...
            resignFirstResponder()
            t_viewController?.showActionSheet(title: "ImageUploadTips", actionTitles: ["ImageStorageRemote","ImageStorageLocal"]) { (index) in
                if index == 0 {
                    self.uploadImage(image)
                } else if index == 1 {
                ...
                if !Configure.shared.showedTips.contains("3") {
                    ...
                    DispatchQueue.main.async {
                        self.t_viewController?.showAlert(title: "Tips", message: "ImageStorageTips", actionTitles: ["GotIt"])
                    }
                }
            }
        }
    ...
    func copyImageToLocal(_ image: UIImage) {
        ...
        let scale = max(min(1024 * 1024.0 / pixelCount,1.0),0.8)
...
//        let pixelCount = image.size.width * image.size.height
//        let scale = max(min(1024 * 1024.0 / pixelCount,1.0),0.6)
//
//        guard let data = image.data(scale) else { return }
//        let md5 = data.md5()
//        let cachePath = Configure.shared.imageCaches[md5] ?? ""
