...
fileprivate var selfWH: CGFloat = {
    return isPad ? 50 : 40
}()
...
extension UIImage: ButtonConvertiable {
    func makeButton() -> UIButton {
        ...
        button.contentEdgeInsets = isPad ? UIEdgeInsets(top: 15, left: 15, bottom: 15, right: 15) : UIEdgeInsets(top: 12, left: 12, bottom: 12, right: 12)
        return button
    }
}
...
class KeyboardBar: UIView {
    ...
    init() {
        super.init(frame: CGRect(x: 0, y: 0, w: windowWidth, h: selfWH))
        ...
        items.forEachEnumerated { (index, item) in
            ...
            button.tag = index
            button.frame = CGRect(x: CGFloat(index) * selfWH, y: 0, w: selfWH, h: selfWH)
            self.scrollView.addSubview(button)
        }
        scrollView.contentSize = CGSize(width: CGFloat(items.count) * selfWH, height: selfWH)
    ...
    override func layoutSubviews() {
        ...
        scrollView.frame = CGRect(x: 0, y: 0, w: w - selfWH, h: selfWH)
        endButton.frame = CGRect(x: w - selfWH, y: 0, w: selfWH, h: selfWH)
    ...
    @objc func tapLink() {
        self.menu?.dismiss(sender: self.menu?.superview as? UIControl)
        textView?.t_viewController?.showAlert(title: "InsertHref", message: "", actionTitles: ["Cancel","OK"], textFieldconfigurationHandler: { (textField) in
